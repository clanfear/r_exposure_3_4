---
title: "R Exposure 4"
subtitle: "Part 2: Mapping"
author: "Chuck Lanfear"
date: "Jan 25, 2020<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
This may require compilation on your computer. If you get errors during install, check with me after class.
...
To get one row per precinct, we want to `spread()`<sup>1</sup> key (`variable`) and value (`estimate`) columns into separate columns for each variable. Then we `mutate()` `Any Black` into a proportion measure.
...
.footnote[[1] `pivot_wider()` is currently broken for `sf` objects, so we must spread. I remove the *margin of error* (`moe`) column before spreading.]
...
* `lwgeom::st_make_valid()` fixes geometry errors from subtraction<sup>1</sup>
* `area_water()` obtains `sf` geometry of water bodies.

Then we can reproduce the same plot using `kc_nowater`...

.footnote[[1] Requires the `lwgeom` package: `install.packages("lwgeom")`]
